...
namespace flocor::py {
    // the product spec
    using slc_meta_t = flocor::products::slc_metadata_t;
    // safe pointers
    using slc_pointer = std::unique_ptr<slc_meta_t>;
...
flocor::py::slc_metadata(py::module & m)
{
    ...
    auto slc = py::class_<slc_meta_t>(m, "SLCMetadata");
    ...
    slc.def(
        py::init([](std::tuple<int, int> pyShape) {
            ...
            // make a shape
            slc_meta_t::layout_type::shape_type shape { lines, samples };
            // use it to make a layout
            slc_meta_t::layout_type layout { shape };
            // and from there an {slc_meta_t} spec
            auto slc = new slc_meta_t(layout);
    ...
    slc.def(
        py::init([](const shape2d_t & shape) {
            // make a layout
            slc_meta_t::layout_type layout { shape };
            // and from that an {slc_meta_t} spec
            auto slc = new slc_meta_t(layout);
            // dress it and return it
            return slc_pointer(slc);
        }),
    ...
    slc.def_property_readonly_static(
        ...
        [](py::object) {
            // easy enough
            return sizeof(slc_meta_t::pixel_type);
        },
        // the docstring
    ...
    slc.def_property_readonly(
        "shape",
        // the getter
        [](const slc_meta_t & slc) { return slc.layout().shape(); },
        // the docstring
